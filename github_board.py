...
def template_to_tape(template, origin):
    """
    Converts template to tape of timestamps, spread across the past year.
    ...
    """
    tape = []
    for j, row in enumerate(template):  # iterate over the weeks
        for i, col in enumerate(row):  # iterate over the days
            if col > 0:
                # Correctly spread commits across different days of the week
                commit_time = origin + (j * STEP) + (i * STEP)  # spread across different days
                tape.extend([commit_time] * col)
    return tape
